...
<p class="text-gray-600">لوحة التحكم المركزية لإدارة الموظفين وأرشفة المستندات والتوكيلات والسيارات والحوادث والبلاغات بقطاع السلام.</p>
</div>
...
<a href="/employees" class="text-custom-red font-semibold text-sm hover:underline">الانتقال للموظفين ←</a>
</div>
<div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:border-custom-red transition">
...
<a href="/archive" class="text-custom-red font-semibold text-sm hover:underline">الانتقال للأرشيف ←</a>
</div>
</div>
</div>
...
{% for cat_name, items in archives.items() %}
...
<span>📂 {{ cat_name }}</span>
<span class="bg-red-50 text-custom-red text-xs px-2 py-1 rounded-full">{{ items|length }}</span>
</h3>
...
جميع الحقوق محفوظة © سلطانة - قطاع السلام 2026
</footer>
{% endif %}
</body>
</html>
...
return render_template_string(HTML_TEMPLATE, active_tab='employees', employees=employees)
...
employees.append({"name": name, "title": title, "file": filename})
return redirect(url_for('employees_page'))

@app.route('/archive')
def archive_page():
if not session.get('logged_in'): return redirect(url_for('index'))
return render_template_string(HTML_TEMPLATE, active_tab='archive', archives=archives)
...
if file and category in archives:
filename = secure_filename(file.filename)
file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
archives[category].append({"title": title, "file": filename})
